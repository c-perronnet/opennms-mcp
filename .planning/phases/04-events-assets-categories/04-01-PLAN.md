...
must_haves:
  truths:
    - "Claude can list events from OpenNMS with an optional FIQL filter (by UEI, node ID, or severity)"
    - "Claude can get a single event by its numeric ID and see full details"
    - "Claude can send a custom event to OpenNMS by specifying a UEI and optional parameters"
    - "Claude can read the full asset record for a node (all fields)"
    - "Claude can update one or more asset fields on a node without clearing other fields"
  artifacts:
    - path: "src/tools/events.ts"
      provides: "registerEventTools() — list_events, get_event, send_event, get_node_asset_record, update_node_asset_record"
      exports: ["registerEventTools"]
    - path: "src/tools/assets.ts"
      provides: "re-exported via events.ts or empty; see action — assets go in events.ts per research recommendation"
    - path: "src/index.ts"
      provides: "Calls registerEventTools() before server.connect()"
      contains: "registerEventTools(server, client, config)"
  key_links:
    - from: "src/index.ts"
      to: "src/tools/events.ts"
      via: "import { registerEventTools } from './tools/events.js'"
      pattern: "registerEventTools"
    - from: "list_events handler"
      to: "client.v2.get('/events')"
      via: "params._s for FIQL, params.limit"
      pattern: "client\\.v2\\.get.*events"
    - from: "send_event handler"
      to: "client.v2.post('/events')"
      via: "JSON body with nodeid/interface/descr field names"
      pattern: "client\\.v2\\.post.*events"
    - from: "get_node_asset_record handler"
      to: "client.v1.get('/nodes/{id}/assetRecord')"
      via: "v1 sub-resource path"
      pattern: "client\\.v1\\.get.*assetRecord"
    - from: "update_node_asset_record handler"
      to: "client.v1.put('/nodes/{id}/assetRecord')"
      via: "URLSearchParams body — BeanWrapper partial update"
      pattern: "client\\.v1\\.put.*assetRecord"
---

<objective>
Create src/tools/events.ts with five MCP tools covering events (read/send) and asset records (read/update), then wire registerEventTools() into src/index.ts.

Purpose: Fulfils EVENT-01, EVENT-02, EVENT-03 (event list, get, send) and ASSET-01, ASSET-02 (asset get, partial update). Assets live in events.ts per the research recommendation — a single registerEventTools() export keeps the wiring symmetrical with existing phases.
Output: src/tools/events.ts (new), src/index.ts (updated), TypeScript compiles clean.
</objective>
...
<interfaces>
<!-- Key types and contracts already established in the codebase. -->
...
export function createApiClient(config: OpenNMSConfig) {
  ...
  return { v2, v1 };  // both AxiosInstance
}
export type ApiClient = ReturnType<typeof createApiClient>;
export function buildErrorMessage(err: unknown, context: string): string;
```

From src/tools/alarms.ts (registerXxxTools pattern to replicate exactly):
```typescript
export function registerAlarmTools(
  server: McpServer,
  client: ApiClient,
  _config: OpenNMSConfig,  // underscore prefix — unused but required for consistent signature
): void { ... }
```

From src/tools/nodes.ts (pattern with multiple tool types, 204 guard, singular array key):
```typescript
// 204 guard pattern:
if (resp.status === 204 || !resp.data?.event?.length) {
  return { content: [{ type: "text", text: "No events found matching the given filter." }] };
}
// URLSearchParams pattern for v1 PUT:
const body = new URLSearchParams(fields);
await client.v1.put(`/nodes/${id}/assetRecord`, body);
// 204 No Content — do NOT access resp.data
```

From src/index.ts (existing wiring pattern):
```typescript
...
<task type="auto">
  <name>Task 1: Create src/tools/events.ts with all five tools</name>
  <files>src/tools/events.ts</files>
  <action>
Create src/tools/events.ts from scratch. This file registers five MCP tools: list_events, get_event, send_event, get_node_asset_record, update_node_asset_record.
...
```
  ...
  <done>src/tools/events.ts exists and TypeScript compilation reports no errors for the file. All five functions (list_events, get_event, send_event, get_node_asset_record, update_node_asset_record) are registered inside registerEventTools().</done>
</task>

<task type="auto">
  <name>Task 2: Wire registerEventTools into src/index.ts</name>
  <files>src/index.ts</files>
  <action>
Edit src/index.ts to import and call registerEventTools(). Add after the existing registerNodeTools lines.

Add the import statement after the existing tool imports:
```typescript
import { registerEventTools } from "./tools/events.js";
```

Add the call after `registerNodeTools(server, client, config);` and before the `server_info` tool registration:
```typescript
// Step 7: Register event and asset tools (Phase 4)
registerEventTools(server, client, config);
```

Update the step numbers below this insertion point accordingly (old Step 7 becomes Step 8, Step 8 becomes Step 9).
...
```
  ...
  <verify>
    <automated>cd /home/c_perronnet/git/opennms-mcp && npx tsc --noEmit 2>&1 | head -20 && grep -n "registerEventTools" src/index.ts</automated>
  </verify>
  <done>src/index.ts imports registerEventTools from ./tools/events.js and calls it with (server, client, config). TypeScript compilation produces no errors.</done>
</task>
...
2. `grep -n "registerEventTools" src/index.ts` shows both the import and the call
3. `grep -n "export function registerEventTools" src/tools/events.ts` confirms the export
4. `grep -n "server.tool" src/tools/events.ts | wc -l` returns 5 (five tools registered)
</verification>

<success_criteria>
- src/tools/events.ts exists with registerEventTools() exporting 5 tools: list_events, get_event, send_event, get_node_asset_record, update_node_asset_record
- send_event body uses: nodeid (lowercase), interface, descr — NOT nodeId/ipAddress/description
- list_events uses client.v2.get("/events") with 204 guard, "event" (singular) array key
- update_node_asset_record uses URLSearchParams body, client.v1.put — no GET-merge-PUT
- src/index.ts calls registerEventTools(server, client, config)
- npx tsc --noEmit exits 0
</success_criteria>

<output>
After completion, create `.planning/phases/04-events-assets-categories/04-01-SUMMARY.md` following the summary template.
</output>
